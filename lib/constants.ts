export const BRAND_COLORS = {
  primary: "#FF0000",
  secondary: "#808080",
}

export const SOCIAL_LINKS = {
  facebook: "https://www.facebook.com/@jannethaguirrebienesraices/?hr=1",
  instagram: "https://www.instagram.com/janneth_aguirrem/",
  tiktok: "https://vm.tiktok.com/ZMhnEwCHp/",
  youtube: "https://www.youtube.com/@jannethaguirrebienesraices5728",
  twitter: "https://twitter.com/?mx=1",
  whatsapp: "https://wa.me/593987167782",
}

export const CONTACT_INFO = {
  email: "info@jannethaguirrebienesraices.com",
  phone: "+593 98 716 7782",
}

export const CATEGORIES = [
  { name: "Ecuador", slug: "ecuador" },
  { name: "Panamá", slug: "panama" },
  { name: "Estados Unidos", slug: "estados-unidos" },
  { name: "Análisis de Mercado", slug: "analisis-de-mercado" },
  { name: "Decoración de Interiores", slug: "decoracion-de-interiores" },
]

export const ARTICLES = [
  {
    id: 1,
    title: "Vizcaya 2: Exclusivo Desarrollo en Ciudad Celeste",
    description:
      "Descubre Vizcaya 2, un moderno desarrollo de departamentos ubicado en la prestigiosa zona de Ciudad Celeste, Samborondón.",
    images: [
      "https://jannethaguirre.online/vizcaya-2-1.jpg",
      "https://jannethaguirre.online/vizcaya-2-2.jpg",
      "https://jannethaguirre.online/vizcaya-2-3.jpg",
    ],
    illustrativeImage: "https://jannethaguirre.online/vizcaya-2-illustrative.jpg",
    category: "ecuador",
    slug: "vizcaya-2-ciudad-celeste",
    subtitle: "Un Oasis de Lujo en Samborondón",
    importantFact: "Vizcaya 2 ofrece un retorno de inversión promedio del 8% anual en alquileres de corto plazo.",
    fullContent: `
      Vizcaya 2 representa la nueva era del lujo residencial en Samborondón, ubicado en el codiciado sector de Ciudad Celeste. Este desarrollo exclusivo ofrece una experiencia de vida incomparable en una de las zonas más prestigiosas de Guayaquil.

      Características destacadas:
      - Ubicación privilegiada en Ciudad Celeste, Samborondón
      - Diseño arquitectónico moderno y elegante
      - Amplios espacios comunes con áreas verdes
      - Estacionamientos cubiertos
      - Seguridad 24/7
      - Acabados de primera calidad
      - Área comercial en planta baja

      El proyecto está estratégicamente ubicado en una de las zonas de mayor plusvalía de Samborondón, con fácil acceso a:
      - Centros comerciales
      - Instituciones educativas de prestigio
      - Restaurantes y zonas de entretenimiento
      - Principales vías de acceso

      Los departamentos de Vizcaya 2 han sido diseñados pensando en el confort y la elegancia, ofreciendo espacios amplios y luminosos con acabados de primera calidad. Cada unidad cuenta con:
      - Amplias áreas sociales
      - Cocina equipada
      - Dormitorios con baño privado
      - Balcones con vistas espectaculares
      - Acabados de lujo

      Este desarrollo representa una excelente oportunidad de inversión en una de las zonas con mayor potencial de valorización en Ecuador.

      Para más información sobre precios y disponibilidad, contáctenos directamente.
    `,
  },
  {
    id: 2,
    title: "OceanLake Golf Villas: Lujo y Naturaleza en Buenaventura, Panamá",
    description:
      "Descubre OceanLake Golf Villas, una colección limitada de villas de lujo en el exclusivo complejo residencial Buenaventura, Panamá.",
    images: [
      "https://jannethaguirre.online/oceanlake-golf-villas-1.jpg",
      "https://jannethaguirre.online/oceanlake-golf-villas-2.jpg",
      "https://jannethaguirre.online/oceanlake-golf-villas-3.jpg",
    ],
    illustrativeImage: "https://jannethaguirre.online/oceanlake-golf-villas-illustrative.jpg",
    category: "panama",
    slug: "oceanlake-golf-villas-buenaventura-panama",
    subtitle: "Exclusividad Costera en Buenaventura",
    importantFact:
      "Las villas incluyen acceso a todas las amenidades de Buenaventura, un complejo de lujo de 450 hectáreas.",
    fullContent: `
      OceanLake Golf Villas representa la cúspide del lujo y la armonía con la naturaleza en el prestigioso complejo residencial Buenaventura, Panamá. Esta colección limitada de villas ofrece una experiencia de vida incomparable, combinando la cercanía al mar con vistas espectaculares al lago y al campo de golf diseñado por Nicklaus.

      Ubicación Privilegiada:
      - Sector OceanLake en Buenaventura
      - A escasos metros de la playa
      - Frente a un majestuoso lago
      - Vista al hoyo No. 5 del campo de golf
      - Cerca del Hotel, Club El Faro y Club Puntarenas

      Detalles de la Propiedad:
      - Vivienda unifamiliar de 3 niveles
      - Área de construcción: 754 m²
      - 4 recámaras y 4.5 baños
      - Cuarto y baño de servicio
      - Den familiar
      - Completamente amoblada y decorada
      - Línea blanca incluida
      - Piscina privada
      - Sistema de aire acondicionado central marca Lennox

      Características por Nivel:
      Planta Baja:
      - Área abierta, semiabierta y cerrada
      - Sala familiar
      - Recámara principal con baño completo y walk-in closet

      Primer Alto:
      - Doble acceso (interno y área de estacionamientos)
      - Sala principal
      - Cocina abierta
      - Recámara de servicio con baño completo
      - Área de lavandería
      - Medio baño de visita
      - Balcón terraza
      - Depósito

      Segundo Alto:
      - Acceso por escalera interna revestida de mármol
      - Área cerrada y semi abierta
      - Amplia terraza
      - Espectacular jacuzzi

      Acabados de Lujo:
      - Pisos revestidos de mármol
      - Cerámica en baños, cocina y lavandería
      - Puertas de acceso de seguridad
      - Cocina con muebles modulares y sobre de granito
      - Electrodomésticos empotrados de alta gama
      - Grifería y artefactos sanitarios importados

      Amenidades de Buenaventura:
      - Campo de golf de 18 hoyos diseñado por Jack Nicklaus
      - Hotel y spa de lujo
      - Restaurantes gourmet
      - Comercios exclusivos
      - Club ecuestre
      - Zoológico privado
      - Marina
      - Clubes de playa
      - Beach resort
      - Parques infantiles
      - Espacios para eventos
      - Sport club
      - 3 km de playa prístina
      - Centro de rescate cultural
      - Actividades acuáticas
      - Capilla
      - Servicio de property management
      - Instituciones educativas de primer nivel

      Buenaventura: Un Paraíso en la Riviera Pacífica
      Buenaventura es un exclusivo desarrollo de 450 hectáreas ubicado en la Riviera Pacífica de Panamá, a solo 80 minutos de la Ciudad de Panamá. Conocido por su impresionante belleza natural, infraestructura de clase mundial y estilo de vida de lujo, Buenaventura ofrece una experiencia incomparable para sus residentes y visitantes.

      El complejo combina perfectamente la belleza natural de la costa pacífica panameña con comodidades de clase mundial, creando un refugio idílico para aquellos que buscan lo mejor en vida residencial y vacacional. Con su campo de golf de campeonato, playas vírgenes, y una variedad de opciones de entretenimiento y relajación, Buenaventura se ha establecido como uno de los destinos más codiciados de Centroamérica.

      Inversión y Oportunidad:
      OceanLake Golf Villas en Buenaventura representa no solo un hogar de ensueño, sino también una sólida oportunidad de inversión. La demanda de propiedades de lujo en destinos de playa premium continúa creciendo, y Buenaventura se posiciona como un líder en este mercado.

      Para aquellos que buscan una residencia de vacaciones, un retiro permanente o una propiedad para generar ingresos por alquiler, OceanLake Golf Villas ofrece una propuesta de valor única. La combinación de su ubicación privilegiada, amenidades de clase mundial y la reputación establecida de Buenaventura asegura que estas villas mantengan y potencialmente aumenten su valor a lo largo del tiempo.

      Conclusión:
      OceanLake Golf Villas en Buenaventura representa la quintaesencia del lujo costero en Panamá. Con su diseño exquisito, ubicación inmejorable y acceso a amenidades de clase mundial, estas villas ofrecen una oportunidad única para aquellos que buscan lo mejor en vida residencial y vacacional. Ya sea como un refugio personal paradisíaco o como una inversión estratégica en el mercado inmobiliario de lujo, OceanLake Golf Villas promete una experiencia sin igual en uno de los destinos más codiciados de Centroamérica.
    `,
  },
  {
    id: 3,
    title: "Millenia Park Orlando: Lujo y Naturaleza en Armonía",
    description:
      "Descubre una nueva era de vida residencial en el corazón de Orlando, donde el lujo se encuentra con la naturaleza en un oasis urbano sin precedentes.",
    images: [
      "https://jannethaguirre.online/millenia-park-orlando-1.jpg",
      "https://jannethaguirre.online/millenia-park-orlando-2.jpg",
      "https://jannethaguirre.online/millenia-park-orlando-3.jpg",
    ],
    illustrativeImage: "https://jannethaguirre.online/millenia-park-orlando-illustrative.jpg",
    category: "estados-unidos",
    slug: "millenia-park-orlando",
    subtitle: "El Oasis Urbano de Orlando",
    importantFact: "Millenia Park ofrece acceso exclusivo a los parques temáticos de Disney World y Universal Studios.",
    fullContent: `
    Bienvenidos a Millenia Park Orlando, un desarrollo residencial que redefine el concepto de vida urbana en el corazón de la Ciudad Bella. Este proyecto visionario fusiona la sofisticación moderna con la serenidad de la naturaleza, creando un refugio único en uno de los destinos más dinámicos de Florida.

    Ubicación Privilegiada:
    Millenia Park Orlando se sitúa estratégicamente en el codiciado distrito de Millenia, ofreciendo a sus residentes una conectividad inigualable:
    - A minutos del Mall at Millenia, epicentro del lujo y la moda
    - Fácil acceso a los parques temáticos de Disney World, Universal Studios y SeaWorld
    - Proximidad al centro de Orlando y su vibrante escena cultural
    - Conexión directa con las principales arterias viales y el Aeropuerto Internacional de Orlando

    Diseño Arquitectónico Innovador:
    Inspirado en la armonía entre el hombre y la naturaleza, Millenia Park Orlando presenta una arquitectura que desafía lo convencional:
    - Edificios de líneas limpias y modernas que se integran con el paisaje natural
    - Uso extensivo de cristal para maximizar la luz natural y las vistas panorámicas
    - Terrazas y balcones amplios que extienden el espacio vital hacia el exterior
    - Áreas comunes diseñadas para fomentar la interacción y el bienestar comunitario

    Residencias de Ensueño:
    Cada apartamento en Millenia Park Orlando es un testimonio de lujo y confort:
    - Unidades desde estudios hasta apartamentos de 3 habitaciones
    - Acabados de primera calidad, incluyendo:
      * Pisos de porcelanato importado
      * Cocinas con encimeras de cuarzo y electrodomésticos de acero inoxidable
      * Baños con ducha tipo lluvia y grifería de diseñador
    - Ventanales del piso al techo que maximizan las vistas y la luz natural
    - Sistemas inteligentes de domótica para control de clima, iluminación y seguridad
    - Closets de diseño personalizado en todas las habitaciones

    Amenidades de Clase Mundial:
    Millenia Park Orlando eleva el estándar de vida con sus amenidades de lujo:
    - Piscina resort-style con cabañas privadas y área de bronceado
    - Gimnasio de última generación con equipos cardiovasculares y de fuerza
    - Spa completo con salas de tratamiento y áreas de relajación
    - Club house con cocina gourmet y espacios para entretenimiento
    - Parque central con senderos para caminar, áreas de picnic y jardines zen
    - Área de juegos para niños y parque para mascotas
    - Estaciones de trabajo compartidas y salas de conferencias

    Compromiso con la Sostenibilidad:
    Millenia Park Orlando no solo ofrece lujo, sino también un compromiso con el medio ambiente:
    - Certificación LEED en proceso
    - Sistemas de riego eficientes y paisajismo con plantas nativas
    - Estaciones de carga para vehículos eléctricos
    - Programa de reciclaje integral y compostaje comunitario
    - Uso de materiales de construcción eco-amigables y de bajo impacto

    Estilo de Vida Incomparable:
    Vivir en Millenia Park Orlando significa abrazar un estilo de vida único:
    - Programa de actividades y eventos comunitarios
    - Clases de yoga y fitness al aire libre
    - Servicio de concierge 24/7
    - Seguridad las 24 horas con acceso controlado
    - Proximidad a restaurantes de clase mundial, tiendas exclusivas y entretenimiento

    Oportunidad de Inversión:
    Millenia Park Orlando representa una oportunidad excepcional para inversores:
    - Ubicado en una de las áreas de mayor crecimiento de Orlando
    - Alto potencial de apreciación debido a la continua expansión del distrito Millenia
    - Demanda constante de alquileres a corto y largo plazo debido a su ubicación estratégica

    Precios y Disponibilidad:
    - Estudios desde $250,000 USD
    - Apartamentos de 1 habitación desde $350,000 USD
    - Apartamentos de 2 habitaciones desde $450,000 USD
    - Apartamentos de 3 habitaciones desde $550,000 USD

    Planes de financiamiento atractivos disponibles para compradores calificados.

    Millenia Park Orlando no es solo un lugar para vivir; es un estilo de vida, una comunidad, y una inversión en el futuro. No pierda la oportunidad de ser parte de este desarrollo excepcional en el corazón de Orlando. Contáctenos hoy para programar una visita personalizada y descubra por qué Millenia Park Orlando es el nuevo estándar de vida de lujo en la Ciudad Bella.
  `,
  },
  {
    id: 4,
    title: "Tendencias en Diseño de Interiores 2025: Lujo Sostenible y Bienestar",
    description:
      "Descubre cómo el diseño de interiores de lujo está adoptando prácticas sostenibles y enfocándose en el bienestar, creando espacios que son tanto elegantes como conscientes del medio ambiente.",
    images: [
      "https://jannethaguirre.online/interior-design-2025-1.jpg",
      "https://jannethaguirre.online/interior-design-2025-2.jpg",
      "https://jannethaguirre.online/interior-design-2025-3.jpg",
    ],
    illustrativeImage: "https://jannethaguirre.online/interior-design-2025-illustrative.jpg",
    category: "decoracion-de-interiores",
    slug: "tendencias-diseno-interiores-2025-lujo-sostenible",
    subtitle: "El Futuro del Diseño de Interiores de Lujo",
    importantFact: "La biofilia y la tecnología inteligente son claves para crear espacios de bienestar en 2025.",
    fullContent: `
    El mundo del diseño de interiores de lujo está experimentando una transformación significativa en 2025, con un enfoque creciente en la sostenibilidad y el bienestar. Esta nueva era de diseño fusiona la opulencia con la responsabilidad ambiental, creando espacios que no solo son visualmente impresionantes, sino también saludables para sus ocupantes y el planeta.

    1. Materiales Sostenibles de Lujo:
       La sostenibilidad ya no está reñida con el lujo. Los diseñadores están optando por:
       - Maderas certificadas FSC y bambú para mobiliario y suelos
       - Mármoles y piedras naturales de origen local para encimeras y revestimientos
       - Textiles orgánicos y reciclados para tapicerías y cortinas
       - Cuero vegano y corcho para acabados y accesorios

    2. Biofilia Elevada:
       La integración de la naturaleza en los interiores alcanza nuevos niveles de sofisticación:
       - Jardines verticales interiores con sistemas de riego automatizados
       - Acuarios de diseño que actúan como elementos arquitectónicos
       - Terrazas y balcones transformados en oasis verdes
       - Uso de formas orgánicas y patrones inspirados en la naturaleza en mobiliario y decoración

    3. Tecnología Inteligente y Sostenible:
       La domótica se centra ahora en la eficiencia energética y el bienestar:
       - Sistemas de iluminación circadiana que se ajustan al ritmo natural del cuerpo
       - Purificadores de aire integrados en los sistemas de climatización
       - Electrodomésticos de ultra bajo consumo controlados por IA
       - Ventanas inteligentes que regulan la luz y la temperatura

    4. Paletas de Color Inspiradas en la Tierra:
       Los tonos naturales y terrosos dominan, creando ambientes serenos y lujosos:
       - Verdes profundos y oliva que evocan bosques y naturaleza
       - Azules oceánicos y grises piedra para un ambiente calmado
       - Marrones ricos y tonos arcilla que aportan calidez
       - Acentos en dorado y bronce para un toque de elegancia

    5. Diseño Multifuncional y Adaptable:
       Los espacios se diseñan para ser flexibles y adaptables a diferentes necesidades:
       - Muebles modulares que pueden reconfigurarse fácilmente
       - Espacios de trabajo integrados que se ocultan cuando no están en uso
       - Áreas de ejercicio que se transforman en zonas de entretenimiento
       - Cocinas que se expanden o contraen según la ocasión

    6. Artesanía Local y Piezas Únicas:
       El lujo se redefine a través de la exclusividad y la historia detrás de cada pieza:
       - Colaboraciones con artesanos locales para crear piezas únicas
       - Incorporación de técnicas tradicionales en diseños modernos
       - Uso de materiales autóctonos en aplicaciones innovadoras
       - Piezas de arte y escultura que cuentan una historia y crean un diálogo con el espacio

    7. Wellness Integrado:
       El diseño de interiores ahora prioriza el bienestar físico y mental:
       - Espacios de meditación y yoga incorporados en el diseño del hogar
       - Saunas y baños de vapor con cromoterapia y aromaterapia
       - Dormitorios diseñados para optimizar el sueño con iluminación y acústica especiales
       - Cocinas equipadas para fomentar una alimentación saludable y sostenible

    8. Minimalismo Cálido:
       Un nuevo enfoque que combina la simplicidad del minimalismo con toques de calidez:
       - Líneas limpias y espacios abiertos característicos del minimalismo
       - Incorporación de texturas ricas y materiales naturales para añadir calidez
       - Uso estratégico de color para crear puntos focales en espacios neutros
       - Iluminación ambiental para crear atmósferas acogedoras en espacios minimalistas

    9. Conexión Interior-Exterior:
       Difuminando los límites entre los espacios interiores y exteriores:
       - Grandes ventanales y puertas corredizas que se abren completamente
       - Materiales y acabados que fluyen sin interrupciones del interior al exterior
       - Muebles de exterior que rivalizan en comodidad y estilo con los de interior
       - Cocinas y salas de estar al aire libre completamente equipadas

    10. Tecnología Invisible:
        La integración perfecta de la tecnología en el diseño:
        - Sistemas de sonido y pantallas que se ocultan cuando no están en uso
        - Cargadores inalámbricos integrados en superficies de muebles
        - Espejos inteligentes que se convierten en pantallas de información
        - Sistemas de control domótico integrados en paneles táctiles discretos

    Conclusión:
    El diseño de interiores de lujo en 2025 se centra en crear espacios que no solo son estéticamente impresionantes, sino que también promueven un estilo de vida sostenible y saludable. La verdadera opulencia se encuentra en la armonía entre la belleza, la funcionalidad y la responsabilidad ambiental. Los diseñadores y propietarios que abrazan estas tendencias están a la vanguardia de una nueva era de lujo consciente, donde el bienestar personal y planetario son la máxima expresión de sofisticación.
    `,
  },
  {
    id: 5,
    title: "Análisis del Mercado Inmobiliario en Ecuador 2025",
    description:
      "Un estudio detallado de las tendencias, oportunidades y desafíos en el mercado inmobiliario ecuatoriano, con datos actualizados y proyecciones para el próximo año.",
    images: [
      "https://jannethaguirre.online/market-analysis-2025-1.jpg",
      "https://jannethaguirre.online/market-analysis-2025-2.jpg",
      "https://jannethaguirre.online/market-analysis-2025-3.jpg",
    ],
    illustrativeImage: "https://jannethaguirre.online/market-analysis-2025-illustrative.jpg",
    category: "analisis-de-mercado",
    slug: "analisis-mercado-inmobiliario-ecuador-2025",
    subtitle: "Perspectivas del Mercado Inmobiliario Ecuatoriano",
    importantFact:
      "Se proyecta un crecimiento del 7% en el sector inmobiliario en ciudades secundarias de Ecuador para 2025.",
    fullContent: `
    El mercado inmobiliario ecuatoriano está experimentando una transformación significativa, impulsada por factores económicos, demográficos y tecnológicos. Nuestro análisis exhaustivo para el 2025 revela tendencias cruciales y oportunidades emergentes que definirán el sector en los próximos años.

    Tendencias Clave:

    1. Crecimiento en Ciudades Secundarias:
       - Cuenca, Manta y Loja están experimentando un boom inmobiliario.
       - Inversores buscan oportunidades fuera de Quito y Guayaquil.

    2. Auge de Proyectos Sostenibles:
       - Incremento en la demanda de edificios con certificación LEED.
       - Desarrollos que incorporan energía solar y sistemas de reciclaje de agua.

    3. Digitalización del Sector:
       - Adopción de tecnologías PropTech para ventas y gestión de propiedades.
       - Aumento en el uso de realidad virtual para visitas inmobiliarias.

    4. Cambios en Preferencias de Vivienda:
       - Mayor demanda de espacios flexibles y áreas de trabajo en casa.
       - Creciente interés en comunidades planificadas con amenidades integradas.

    5. Inversión Extranjera:
       - Aumento de compradores internacionales, especialmente de Estados Unidos y Europa.
       - Atractivo de Ecuador como destino de retiro y segunda residencia.

    Desafíos y Oportunidades:

    - Regulaciones: Nuevas políticas gubernamentales que podrían afectar al sector.
    - Financiamiento: Evolución de las opciones de crédito hipotecario.
    - Infraestructura: Proyectos de mejora urbana que impactarán valores inmobiliarios.

    Para un análisis interactivo detallado, incluyendo gráficos, proyecciones y datos específicos por región, visite nuestra página de Análisis de Mercado completo.

    [Enlace al Análisis Interactivo](https://jannethaguirre.com/analisis-de-mercado.html)

    Este análisis es una herramienta invaluable para inversores, desarrolladores y compradores que buscan tomar decisiones informadas en el dinámico mercado inmobiliario ecuatoriano. Manténgase actualizado con las últimas tendencias y oportunidades para maximizar su potencial de inversión.
    `,
  },
  {
    id: 6,
    title: "Mocolí Bay: Lujo Frente al Río en Samborondón",
    description:
      "Descubre la vida exclusiva en Mocolí Bay, un desarrollo de apartamentos de lujo con vistas panorámicas al río Daule.",
    images: [
      "https://jannethaguirre.online/mocoli-bay-1.jpg",
      "https://jannethaguirre.online/mocoli-bay-2.jpg",
      "https://jannethaguirre.online/mocoli-bay-3.jpg",
    ],
    illustrativeImage: "https://jannethaguirre.online/mocoli-bay-illustrative.jpg",
    category: "ecuador",
    slug: "mocoli-bay-apartamentos-samborondon",
    subtitle: "Vistas Panorámicas al Río Daule",
    importantFact: "Mocolí Bay ofrece un muelle privado para embarcaciones y acceso a las mejores amenidades.",
    fullContent: `
    Bienvenido a Mocolí Bay, la nueva joya residencial de Samborondón que redefine el lujo frente al río. Este exclusivo desarrollo de apartamentos ofrece una experiencia de vida sin precedentes, combinando la serenidad de las vistas al río Daule con la comodidad de la vida urbana moderna.

    Ubicación Privilegiada:
    Mocolí Bay se encuentra estratégicamente ubicado en el corazón de Samborondón, una de las zonas más prestigiosas y de mayor crecimiento en Guayaquil. A solo minutos de centros comerciales, restaurantes de clase mundial y las mejores instituciones educativas, este desarrollo ofrece lo mejor de ambos mundos: tranquilidad y accesibilidad.

    Diseño Arquitectónico de Vanguardia:
    El edificio, diseñado por reconocidos arquitectos, se eleva majestuosamente sobre el paisaje ribereño. Su fachada de cristal y acero refleja la luz del sol y las aguas del río, creando un espectáculo visual que cambia a lo largo del día.

    Apartamentos de Ensueño:
    - Amplios espacios que van desde los 100m² hasta los 300m²
    - Ventanales del piso al techo que maximizan las vistas panorámicas
    - Acabados de lujo, incluyendo mármol italiano y maderas nobles
    - Cocinas gourmet equipadas con electrodomésticos de última generación
    - Sistemas de domótica para control de iluminación, clima y seguridad

    Amenidades de Clase Mundial:
    - Piscina infinity con vistas al río Daule
    - Gimnasio completamente equipado con tecnología de punta
    - Spa y salas de masaje para relajación total
    - Salón de eventos con capacidad para grandes celebraciones
    - Muelle privado para embarcaciones
    - Áreas verdes y jardines paisajísticos

    Sostenibilidad y Tecnología:
    Mocolí Bay no solo es un símbolo de lujo, sino también de responsabilidad ambiental. El edificio incorpora:
    - Paneles solares para áreas comunes
    - Sistema de recolección de agua de lluvia
    - Materiales de construcción eco-amigables
    - Estaciones de carga para vehículos eléctricos

    Inversión Inteligente:
    Con el constante crecimiento de Samborondón, Mocolí Bay representa no solo un hogar de ensueño, sino también una inversión sólida con alto potencial de apreciación.

    Precios desde $350,000 USD

    No pierda la oportunidad de ser parte de este desarrollo exclusivo. Las unidades están disponibles en pre-venta con atractivos planes de financiamiento. Contáctenos hoy para agendar una visita al showroom y descubrir por qué Mocolí Bay es el futuro del lujo residencial en Ecuador.
  `,
  },
  {
    id: 7,
    title: "Samborondón City Center: El Nuevo Epicentro de Negocios y Estilo de Vida",
    description:
      "Explore las oportunidades únicas en el corazón de Samborondón, donde el comercio y el lujo se fusionan en un desarrollo de uso mixto sin precedentes.",
    images: [
      "https://jannethaguirre.online/samborondon-city-center-1.jpg",
      "https://jannethaguirre.online/samborondon-city-center-2.jpg",
      "https://jannethaguirre.online/samborondon-city-center-3.jpg",
    ],
    illustrativeImage: "https://jannethaguirre.online/samborondon-city-center-illustrative.jpg",
    category: "ecuador",
    slug: "samborondon-city-center",
    subtitle: "El Corazón de Samborondón",
    importantFact:
      "Samborondón City Center ofrece una ubicación estratégica con acceso directo a las principales vías viales.",
    fullContent: `
    Bienvenidos a Samborondón City Center, el proyecto que está transformando el skyline y redefiniendo el concepto de vida urbana en el corazón de Samborondón. Este desarrollo de uso mixto representa la culminación de años de crecimiento y visión para esta exclusiva zona de Guayaquil.

    Un Nuevo Ícono Urbano:
    Samborondón City Center se eleva como un complejo de torres que alberga espacios comerciales, oficinas de lujo y residencias exclusivas. Su diseño vanguardista, obra de un equipo de arquitectos de renombre internacional, fusiona funcionalidad con estética, creando un landmark que se convertirá en el nuevo símbolo de Samborondón.

    Ubicación Estratégica:
    Situado en el kilómetro 1.5 de la vía a Samborondón, este desarrollo ofrece una conectividad inigualable:
    - A minutos del aeropuerto José Joaquín de Olmedo
    - Acceso directo a las principales arterias viales
    - Rodeado de los mejores centros comerciales, restaurantes y servicios

    Espacios Comerciales de Vanguardia:
    - Locales comerciales desde 50m² hasta 500m²
    - Amplios corredores y plazas para alto tráfico peatonal
    - Fachadas de cristal para máxima visibilidad
    - Infraestructura tecnológica de punta

    Oficinas Premium:
    - Plantas libres desde 100m² hasta pisos completos de 1000m²
    - Sistemas de seguridad biométrica
    - Salas de conferencias compartidas
    - Estacionamiento con valet parking

Residencias de Lujo:
    - Apartamentos desde 80m² hasta 300m²
    - Acabados de primera, incluyendo pisos de mármol y cocinas italianas
    - Vistas panorámicas de la ciudad y el río Daule
    - Amenidades exclusivas: piscina infinity, gimnasio, spa y sky lounge

Tecnología y Sostenibilidad:
Samborondón City Center no solo es un proyecto de lujo, sino también un ejemplo de construcción sostenible:
    - Certificación LEED en proceso
    - Sistemas de ahorro energético y de agua
    - Áreas verdes y jardines verticales que mejoran la calidad del aire
    - Estaciones de carga para vehículos eléctricos

Oportunidad de Inversión:
Este desarrollo representa una oportunidad única para inversores visionarios:
    - Alto potencial de valorización
    - Rentas atractivas para espacios comerciales y de oficinas
    - Demanda garantizada en una de las zonas de mayor crecimiento de Ecuador

Precios:
    - Locales comerciales desde $3,500/m²
    - Oficinas desde $2,800/m²
    - Residencias desde $2,500/m²

Samborondón City Center no es solo un lugar para vivir, trabajar o hacer negocios; es un estilo de vida, una declaración de éxito y una mirada hacia el futuro de Samborondón. No pierda la oportunidad de ser parte de este proyecto emblemático. Contáctenos hoy para obtener más información y agendar una visita personalizada a nuestro showroom.
    `,
  },
  {
    id: 8,
    title: "Casolé: Lujo Contemporáneo en el Corazón de Quito",
    description:
      "Descubre la fusión perfecta entre diseño vanguardista y comodidad en el nuevo proyecto residencial de Uribe & Schwarzkopf en la capital ecuatoriana.",
    images: [
      "https://jannethaguirre.online/casole-quito-1.jpg",
      "https://jannethaguirre.online/casole-quito-2.jpg",
      "https://jannethaguirre.online/casole-quito-3.jpg",
    ],
    illustrativeImage: "https://jannethaguirre.online/casole-quito-illustrative.jpg",
    category: "ecuador",
    slug: "casole-quito",
    subtitle: "Diseño Contemporáneo en el Corazón de Quito",
    importantFact: "Casolé ofrece una piscina infinity con vistas panorámicas de la ciudad y los volcanes.",
    fullContent: `
    Bienvenidos a Casolé, la nueva joya arquitectónica que está redefiniendo el skyline de Quito. Desarrollado por el prestigioso grupo Uribe & Schwarzkopf, Casolé representa la cúspide del diseño contemporáneo y el lujo urbano en la capital ecuatoriana.

    Ubicación Privilegiada:
    Casolé se erige majestuosamente en el corazón del exclusivo barrio de La Carolina, ofreciendo a sus residentes una conexión inigualable con lo mejor de Quito:
    - A pasos del Parque La Carolina, el pulmón verde de la ciudad
    - Rodeado de los principales centros financieros y comerciales
    - Acceso inmediato a restaurantes de clase mundial y vida nocturna vibrante
    - Proximidad a centros culturales, museos y galerías de arte

    Diseño Arquitectónico de Vanguardia:
    Concebido por un equipo de arquitectos de renombre internacional, Casolé se distingue por su diseño audaz y sostenible:
    - Fachada dinámica con elementos geométricos que juegan con la luz natural
    - Amplios balcones y terrazas que difuminan los límites entre el interior y el exterior
    - Uso innovador de materiales eco-amigables que respetan el entorno urbano

    Residencias de Ensueño:
    Casolé ofrece una variedad de opciones para satisfacer los gustos más exigentes:
    - Apartamentos desde 60m² hasta 250m²
    - Áticos de lujo con terrazas panorámicas
    - Acabados de primera calidad, incluyendo:
      * Pisos de madera de ingeniería
      * Cocinas italianas con electrodomésticos de última generación
      * Baños con grifería de diseñador y mármol importado
    - Sistemas de domótica para control de iluminación, clima y seguridad

    Amenidades de Clase Mundial:
    Los residentes de Casolé disfrutan de un estilo de vida comparable al de un resort de cinco estrellas:
    - Piscina infinity con vistas panorámicas de la ciudad y los volcanes
    - Gimnasio equipado con tecnología de punta y estudios para clases grupales
    - Spa completo con sauna, jacuzzi y salas de masaje
    - Salón de eventos con cocina gourmet
    - Cine privado y sala de juegos
    - Coworking spaces y salas de reuniones
    - Áreas verdes y jardines zen para meditación

    Sostenibilidad y Tecnología:
    Casolé no solo es un símbolo de lujo, sino también de responsabilidad ambiental:
    - Certificación LEED en proceso
    - Sistemas de recolección de agua lluvia y tratamiento de aguas grises
    - Paneles solares para áreas comunes
    - Estaciones de carga para vehículos eléctricos
    - Bicicletas eléctricas compartidas para residentes

    Inversión con Visión de Futuro:
    Adquirir una residencia en Casolé no es solo comprar un hogar, es invertir en un estilo de vida y en el futuro de Quito:
    - Valorización garantizada en una de las zonas de mayor plusvalía de la ciudad
    - Oportunidad de generar ingresos a través de alquileres de corto plazo
    - Respaldo de Uribe & Schwarzkopf, líder en desarrollo inmobiliario en Ecuador

    Precios y Financiamiento:
    - Apartamentos desde $200,000 USD
    - Áticos y unidades premium desde $500,000 USD
    - Planes de financiamiento flexibles con entidades bancarias de primer nivel
    - Opciones de pago diferido durante la construcción

    Casolé no es solo un edificio; es una obra de arte habitable, un oasis urbano que eleva el estándar de vida en Quito. No pierda la oportunidad de ser parte de este proyecto emblemático. Contáctenos hoy para agendar una visita a nuestro showroom y experimente el futuro del lujo residencial en Ecuador.
    `,
  },
]

